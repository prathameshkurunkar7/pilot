...
- **No Docker.** Services (MariaDB, PostgreSQL, Redis) are installed directly on the host via apt (Ubuntu) or Homebrew (macOS). MariaDB is the default site engine; PostgreSQL is installed too and available per-site.
