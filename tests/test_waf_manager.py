...
from pilot.managers import waf_manager
...
def _crs_tarball() -> bytes:
    ...
    buffer = io.BytesIO()
    top = f"coreruleset-{waf_manager.CRS_VERSION}"
...
def fake_download(monkeypatch):
    ...
    ran: list = []
    monkeypatch.setattr(waf_manager, "run_command", lambda cmd: ran.append(cmd))

    def _install(payload: bytes, expected_sha: str | None = None):
        if expected_sha is not None:
            monkeypatch.setattr(waf_manager, "_CRS_SHA256", expected_sha)

        def _urlretrieve(url, dest):
            Path(dest).write_bytes(payload)
        monkeypatch.setattr(waf_manager.urllib.request, "urlretrieve", _urlretrieve)
        return ran
...
def test_install_crs_accepts_matching_archive(fake_download) -> None:
    ...
    assert any(str(waf_manager.SHARED_MODSEC_DIR / "rules") in " ".join(map(str, cmd)) for cmd in ran)


def test_pinned_digest_is_a_sha256() -> None:
    assert len(waf_manager._CRS_SHA256) == 64
    int(waf_manager._CRS_SHA256, 16)  # hex
