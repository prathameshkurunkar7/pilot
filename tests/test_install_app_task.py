"""Tests for admin.backend.tasks.jobs.install_app_task.InstallAppTask.
...
from admin.backend.tasks.jobs.install_app_task import InstallAppTask
...
def test_install_app_task_uses_site_install_app(tmp_path: Path) -> None:
    ...
    with patch.object(Site, "install_app") as mock_install, \
            patch("pilot.managers.python_env_manager.PythonEnvManager.build_assets_for_app"):
        task.run()
