...
class SystemdProcessManager(ProcessManager):
    """Manages bench processes via systemd (used in production on Atlas/Ubuntu)."""
    ...
    def install_config(self) -> None:
        ...
        for unit in units:
            ...
            dst = self.system_unit_dir / unit
            if dst.is_symlink() or dst.exists():
                dst.unlink()
            dst.symlink_to(src)
        run_command(["systemctl", "daemon-reload"])
        run_command(["systemctl", "enable", self._target_name()])

    def is_configured(self) -> bool:
        result = subprocess.run(
            ["systemctl", "is-enabled", self._target_name()],
            capture_output=True,
        )
    ...
    def start(self) -> None:
        ...
        self.reload()
        run_command(["systemctl", "start", self._target_name()])

    def stop(self) -> None:
        run_command(["systemctl", "stop", self._target_name()])

    def restart(self) -> None:
        run_command(["systemctl", "restart", self._target_name()])

    def is_running(self) -> bool:
        result = subprocess.run(
            ["systemctl", "is-active", self._target_name()],
            capture_output=True,
        )
        return result.returncode == 0
    ...
    def reload_web(self) -> None:
        ...
        if self.is_running():
            ...
            run_command(["systemctl", "restart", self._unit_name("web")])
